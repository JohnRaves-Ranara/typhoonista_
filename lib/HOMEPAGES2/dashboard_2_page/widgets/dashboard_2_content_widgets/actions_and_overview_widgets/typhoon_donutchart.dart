...
class _typhoon_donutchartState extends State<typhoon_donutchart> {
                    ...
                    if (snapshot.hasData) {
                      ...
                      return StreamBuilder<List<Municipality>>(
                        stream: FirestoreService2()
                            .streamAllMunicipalities(snapshot.data!),
                        builder: (context, snapshot) {
                          if (snapshot.hasData) {
                            ...
                            for (int i = 0; i < municipalities.length; i++) {
                              municipalities[i].damageCostInPercentage =
                                  double.parse(
                                      damagePercentages[i].toStringAsFixed(0));
                            }
                            return Row(children: [
                              Expanded(
                                flex: 60,
                  ...
                  builder: (context, snapshot) {
                    if (snapshot.hasData) {
                      return StreamBuilder<List<Province>>(
                        stream: FirestoreService2()
                            .streamAllProvinces(snapshot.data!),
                        builder: (context, snapshot) {
  ...
  List<double> municipalConvertToPercentages(
      List<Municipality> municipalities) {
    double dmgCosts = municipalities
        .map((mun) => mun.totalDamageCost)
        .fold(0, (a, b) => a + b);
    List<double> percentages = municipalities
        .map((mun) => (mun.totalDamageCost / dmgCosts) * 100)
        .toList();
    return percentages;
  }
  ...
  List<double> provincialConvertToPercentages(List<Province> provinces) {
    double dmgCosts =
        provinces.map((prov) => prov.totalDamageCost).fold(0, (a, b) => a + b);
    List<double> percentages = provinces
        .map((prov) => (prov.totalDamageCost / dmgCosts) * 100)
        .toList();
    return percentages;
  }
}
